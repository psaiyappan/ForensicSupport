...
FileList construct_path_list( int argc, char ** argv, char const * filename ) {
	...
	FILE * file = 0;
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Androido1: FileName %s", filename);
	if (!filename) {
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Androido1: No file");
	}
	else if (!strcmp(filename,"-")) {
		file = stdin;
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Androido1: - file");
	}
	else {
		file = fopen( filename, "r" );
		if(file == NULL) {
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Androido1: Can't open file");
		}
		else
		{
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Androido1: Success open file");
		}
	}
